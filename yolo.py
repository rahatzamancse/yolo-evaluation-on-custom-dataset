# USAGE
# python yolo.py --images input_images --yolo yolo-coco
...
# labelsPath = args["yolo"] + "/coco.names"
...
weightsPath = os.path.sep.join([args["yolo"], "weights.weights"])
configPath = os.path.sep.join([args["yolo"], "config.cfg"])
...
bikes = {}
# loop in the dir
images = list(os.listdir(args["images"]))
for i in tqdm(range(len(images))):
    ...
    image_path = cv2.imread(args["images"] + '/' + filename)
    ...
    if len(idxs) > 0:
        ...
        for i in idxs.flatten():
            ...
            # clss[filename]['class'].append(LABELS[classIDs[i]])
    ...
    cv2.imwrite('output/' + filename, image_path)
    # cv2.waitKey(0)
